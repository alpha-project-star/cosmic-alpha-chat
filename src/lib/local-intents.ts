...
export function tryLocalIntent(raw: string): string | null {
  ...
  // ---- BULK CLEAR -------------------------------------------------------
  mm = lower.match(/^(?:delete|remove|clear)\s+all\s+(notes|reminders|memories|memory|plans|bills)/);
  if (mm) {
    const kind = mm[1].replace(/s$/, "");
  ...
  if (/^(?:clear|delete|remove)\s+(?:all\s+)?done\s+reminders/.test(lower)) {
    ...
    let n = 0;
    for (const r of s.reminders) if (r.done === "yes") { alphaStore.deleteReminder(r.id); n++; }
    return `Cleared ${n} completed reminder${n === 1 ? "" : "s"}.`;
  }

  // ---- DELETE BY NAME (fuzzy) ------------------------------------------
  mm = lower.match(/^(?:delete|remove|forget)\s+(?:the\s+)?(note|reminder|memory|plan|bill)\s+(?:about\s+|called\s+|named\s+|to\s+)?(.+)$/);
  if (mm) return deleteFuzzy(mm[1], trim(mm[2]));
  mm = lower.match(/^forget\s+(?:that|about)\s+(.+)$/);
  if (mm) return deleteFuzzy("memory", trim(mm[1]));

  // ---- UPDATE ----------------------------------------------------------
  mm = lower.match(/^(?:rename|change)\s+(?:the\s+)?note\s+(?:called\s+|named\s+)?(.+?)\s+to\s+(.+)$/);
  if (mm) {
    const q = trim(mm[1]); const to = trim(mm[2]);
...
function listItems(kind: string): string {
  ...
  if (kind === "reminder") {
    ...
    return "**Reminders:**\n" + s.reminders.slice(0, 20).map(r =>
      `• ${r.title}${r.when ? " @ " + r.when : ""}${r.done === "yes" ? " ✅" : ""}`).join("\n");
  }
  if (kind === "note") {
    if (!s.notes.length) return "You have no notes.";
    return "**Notes:**\n" + s.notes.slice(0, 20).map(n =>
      `• ${n.title || "(untitled)"}: ${(n.body || "").slice(0, 80)}`).join("\n");
  }
  if (kind === "memory") {
    if (!s.memories.length) return "No memories stored yet.";
    return "**Memories:**\n" + s.memories.slice(0, 20).map(m =>
      `• ${m.topic}: ${m.detail}`).join("\n");
  }
  if (kind === "plan") {
    if (!s.plans.length) return "You have no plans.";
    return "**Plans:**\n" + s.plans.slice(0, 20).map(p =>
      `• ${p.title} (${p.from} → ${p.to})${p.date ? " on " + p.date : ""}`).join("\n");
  }
  if (kind === "bill") {
    if (!s.bills.length) return "You have no bills.";
    return "**Bills:**\n" + s.bills.slice(0, 20).map(b =>
      `• ${b.name}: $${b.balance} (${b.status})${b.dueDate ? " due " + b.dueDate : ""}`).join("\n");
  }
...
function bulkClear(kind: string): string {
  ...
  if (kind === "note") { list = s.notes; del = alphaStore.deleteNote; }
  else if (kind === "reminder") { list = s.reminders; del = alphaStore.deleteReminder; }
  else if (kind === "memory") { list = s.memories; del = alphaStore.deleteMemory; }
  else if (kind === "plan") { list = s.plans; del = alphaStore.deletePlan; }
  else if (kind === "bill") { list = s.bills; del = alphaStore.deleteBill; }
...
function deleteFuzzy(kind: string, q: string): string {
  ...
  if (kind === "note") {
    const matches = s.notes.filter(n => (n.title || "").toLowerCase().includes(lc) || (n.body || "").toLowerCase().includes(lc));
    if (!matches.length) return `No note matching "${q}".`;
    if (matches.length > 1) return `Multiple notes match "${q}" — which one? (${matches.map(m => m.title || m.body.slice(0, 20)).join(", ")})`;
  ...
  if (kind === "reminder") {
    ...
    if (matches.length > 1) return `Multiple reminders match "${q}" — which? (${matches.map(m => m.title).join(", ")})`;
    alphaStore.deleteReminder(matches[0].id);
    return `Deleted reminder "${matches[0].title}".`;
  }
  if (kind === "memory") {
    const matches = s.memories.filter(m => m.topic.toLowerCase().includes(lc) || m.detail.toLowerCase().includes(lc));
    if (!matches.length) return `No memory matching "${q}".`;
    if (matches.length > 1) return `Multiple memories match "${q}" — which? (${matches.map(m => m.topic).join(", ")})`;
...
function parseNaturalWhen(raw: string): string {
  ...
  if (m) {
    const n = Number(m[1]);
    const unit = m[2];
    const ms = /second/.test(unit) ? n * 1000
