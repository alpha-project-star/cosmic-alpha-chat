...
function tx<T>(mode: IDBTransactionMode, run: (store: IDBObjectStore) => IDBRequest<T>): Promise<T> {
  return openDb().then(db => new Promise<T>((resolve, reject) => {
    ...
    transaction.onerror = () => { db.close(); reject(transaction.error || new Error("Music transaction failed.")); };
  }));
}
...
export async function playMusicByName(query = ""): Promise<string> {
  const rows = await tx<any[]>("readonly", store => store.getAll());
  if (!rows.length) return "No music is stored yet. Upload MP3 files in Settings → Alpha Data → Music Library.";
  const q = query.trim().toLowerCase();
  const row = q
    ? rows.find(r => String(r.name || "").toLowerCase().includes(q))
