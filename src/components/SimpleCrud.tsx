              ...
              {f.type === "textarea" ? (
                <>
                  ...
                  <div className="text-[11px] text-muted-foreground mt-1 leading-[1.4]">
                    Formatting (headings, lists, tables, code, maths) is preserved and rendered below once saved.
                  </div>
                </>
              ) : f.type === "select" ? (
        ...
        {items.map(item => (
          <div key={item.id} className="glass rounded-xl p-4">
              ...
              {fields.map(f => {
                ...
                return (
                  <div key={f.key} className={isLong ? "col-span-2 min-w-0" : "min-w-0"}>
                    <div className="text-xs uppercase tracking-wider text-muted-foreground leading-[1.4]">{f.label}</div>
                    {isLong ? (
                      raw ? (
                        <div className="mt-1.5"><RichText text={String(raw)} /></div>
                      ) : (
                        <div className="text-sm text-muted-foreground mt-1">—</div>
                      )
                    ) : (
                      <div className="text-[15px] leading-[1.5] text-foreground/90 break-words mt-0.5">
                        {f.prefix}{raw === undefined || raw === null || raw === "" ? "—" : String(raw)}{f.suffix}
                      </div>
                    )}
                  </div>
                );
            ...
            <div className="flex gap-2 justify-end mt-3">
              <button onClick={() => speakItem(item)} aria-label="Read aloud"
