  ...
  return (
    <HudPanel className="flex-1 min-h-0 flex flex-col p-4">
      <div className="flex items-center justify-between mb-3 pr-16">
        ...
        <Link to="/chat" className="hud-bubble text-[10px] wordmark px-2 py-1 inline-flex items-center gap-1">
      ...
      <div ref={scrollRef} className="flex-1 min-h-0 overflow-y-auto space-y-3 pr-1">
        {recent.length === 0 && (
          <div className="text-xs text-muted-foreground italic">
            No exchanges yet. Tap the mic and speak — say <span className="text-primary">"open chat"</span> or ask anything.
          </div>
        )}
        {recent.map(m => (
